...
static int32_t gpio_cfg(struct platform_device *pdev, enum ert_gpio_cfg type)
{
	...
	xdev_handle_t xdev = xocl_get_xdev(cfg_gpio->pdev);
	int32_t ret = 0, val = 0;
	int i;
	...
	case INTR_TO_ERT:
		...
		#if 0
		for (i = 0; i < cfg_gpio->num_slots; i++)
	...
	case MB_SLEEP:
		...
		#if 0
		iowrite32(ERT_EXIT_CMD, cfg_gpio->cq_base);
		#endif
...
static int config_gpio_remove(struct platform_device *pdev)
{
	...
	//sysfs_remove_group(&pdev->dev.kobj, &config_gpio_attr_group);
